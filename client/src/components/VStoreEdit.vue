...
export default {
  ...
  watch: {
    ...
    dialog (v) {
      if (v) {
        this.credentials = this.store.credentials
        this.storeMeta = { name: this.store.name }
        this.platform = this.platforms.find(pl => pl.text === this.store.platform).value
      }
    }
  },
  ...
  methods: {
    async handleSubmit () {
      ...
      await firebase.database()
        .ref(`stores/${this.me.uid}/${this.store.key}`)
        .set({
          ...this.storeMeta,
